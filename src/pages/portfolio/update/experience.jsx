...
import { clearErrors } from "@/redux/reducers/experienceDataReducer";
import {
  resetExperience,
...
const Experience = () => {
  ...
  useEffect(() => {
    if (experienceUpdated) {
      showAlert("Experience data upadated successfully !", "success");
  ...
  useEffect(() => {
    if (user?.uid) {
      dispatch(authGetExperience(user.uid));
    } else {
      router.push("/");
    }
  ...
  useEffect(() => {
    if (experience?.userName) {
      setExperienceArray(experience.experiences);
    }
  }, [experience]);

  useEffect(() => {
    const { companyName } = experienceForm;
    if (!companyName) {
      window.scrollTo({
  ...
  }, [experienceArray, experienceForm]);
      ...
      {!user?.email || loading ? (
        ...
        <>
          <div className='px-[20px] pb-[30px] pt-[80px] m-auto min-h-[100vh] md:w-[600px]'>
            {/* Form Heading */}
            <h2 className='text-3xl text-center mb-3'>Update Experience</h2>
